...
def linear_fp16(x: torch.Tensor, weight: Weight, out: torch.Tensor | None = None) -> torch.Tensor:
...
def fp16_embedding(ids: torch.Tensor, table: Weight | torch.Tensor, out: torch.Tensor | None = None,
                   validate_ids: bool = False) -> torch.Tensor:
    ...
    if isinstance(table, Weight) and (table.kind != "fp16" or len(table.logical_shape) != 2):
