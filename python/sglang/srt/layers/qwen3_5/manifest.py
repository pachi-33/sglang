...
def _fp8(n: int, k: int) -> dict[str, HeaderSpec]:
    return {"weight": HeaderSpec((n, k), "F8_E4M3"), "weight_scale": HeaderSpec((n // 128, k // 128), "F16")}
...
def expected_headers() -> dict[str, HeaderSpec]:
    ...
    gdn = {
        "linear_attn.A_log": HeaderSpec((32,), "F16"), "linear_attn.dt_bias": HeaderSpec((32,), "F16"),
        "linear_attn.conv1d.weight": HeaderSpec((8192, 1, 4), "F16"),
        "linear_attn.in_proj_a.weight": HeaderSpec((32, 2048), "F16"), "linear_attn.in_proj_b.weight": HeaderSpec((32, 2048), "F16"),
        "linear_attn.norm.weight": HeaderSpec((128,), "F16"),
    }
    for name, (n, k) in {"in_proj_qkv": (8192, 2048), "in_proj_z": (4096, 2048), "out_proj": (2048, 4096)}.items():
        for suffix, spec in _fp8(n, k).items(): gdn[f"linear_attn.{name}.{suffix}"] = spec
    full = {"self_attn.q_norm.weight": HeaderSpec((256,), "F16"), "self_attn.k_norm.weight": HeaderSpec((256,), "F16")}
    for name, (n, k) in {"q_proj": (8192, 2048), "k_proj": (512, 2048), "v_proj": (512, 2048), "o_proj": (2048, 4096)}.items():
        for suffix, spec in _fp8(n, k).items(): full[f"self_attn.{name}.{suffix}"] = spec
    for layer in range(40):
        for relative, spec in (common | (gdn if layer % 4 != 3 else full)).items(): out[f"{P}layers.{layer}.{relative}"] = spec
        for expert in range(256):
            for projection in ("gate_proj", "up_proj", "down_proj"): out.update(_expert(layer, expert, projection))
    return out
