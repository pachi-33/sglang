...
@triton.jit
def _full_qk_gate_kernel(qg, k, positions, q_weight, k_weight, q_out, k_out, gate_out,
                         tokens: tl.constexpr, SQ0: tl.constexpr, SQ1: tl.constexpr,
                         SK0: tl.constexpr, SK1: tl.constexpr, SP: tl.constexpr,
                         SWQ: tl.constexpr, SWK: tl.constexpr, theta: tl.constexpr):
    ...
    qval = (qval * tl.math.rsqrt(qss / 256.0 + 1.0e-6) * qscale).to(tl.float16).to(tl.float32)
    kval = tl.load(kval).to(tl.float32)
    kss = tl.sum(kval * kval, axis=0)
    kscale = tl.load(k_weight + d * SWK).to(tl.float32) + 1.0
    kval = (kval * tl.math.rsqrt(kss / 256.0 + 1.0e-6) * kscale).to(tl.float16).to(tl.float32)
    pair = d % 32
    ...
    qmate = (qmate * tl.math.rsqrt(qss / 256.0 + 1.0e-6) * (tl.load(q_weight + mate * SWQ).to(tl.float32) + 1.0)).to(tl.float16).to(tl.float32)
    kmate = (kmate * tl.math.rsqrt(kss / 256.0 + 1.0e-6) * (tl.load(k_weight + mate * SWK).to(tl.float32) + 1.0)).to(tl.float16).to(tl.float32)
...
def full_qk_rope_gate(q_gate: torch.Tensor, k: torch.Tensor, positions: torch.Tensor,
                      q_norm: torch.Tensor, k_norm: torch.Tensor,
                      theta: float = 10_000_000.0) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    """
    if (q_gate.ndim != 2 or q_gate.shape[1] != 8192 or k.shape != (q_gate.shape[0], 512)
            or q_gate.dtype != torch.float16 or k.dtype != torch.float16):
        raise ValueError("expected FP16 q_gate[T,8192] and k[T,512]")
    if not all(t.is_cuda and t.is_contiguous() for t in (q_gate, k, positions, q_norm, k_norm)):
        raise ValueError("full_qk_rope_gate requires contiguous CUDA tensors")
    ...
    if positions.shape != (q_gate.shape[0],) or positions.dtype not in (torch.int32, torch.int64):
        raise ValueError("positions must be CUDA integer [T]")
    if q_norm.shape != (256,) or k_norm.shape != (256,) or q_norm.dtype != torch.float16 or k_norm.dtype != torch.float16:
    ...
    if t:
        _full_qk_gate_kernel[(t, 16)](q_gate, k, positions, q_norm, k_norm, q, kout, gate,
                                      t, q_gate.stride(0), q_gate.stride(1), k.stride(0), k.stride(1),
                                      positions.stride(0), q_norm.stride(0), k_norm.stride(0), theta,
...
def split_full_v(v: torch.Tensor) -> torch.Tensor:
    ...
    if v.ndim != 2 or v.shape[1] != 512 or v.dtype != torch.float16 or not v.is_cuda or not v.is_contiguous():
...
@triton.jit
def _gated_attention_a8_kernel(attn, gate, q, scale, groups: tl.constexpr, BLOCK: tl.constexpr):
    token = tl.program_id(0)
    ...
    tl.store(q + offset, _encode_e4m3fn(tl.math.div_rn(half_value.to(tl.float32), safe)))
    tl.store(scale + token * groups + group, scale_value)


def gated_attention_fp8(attention: torch.Tensor, gate: torch.Tensor):
    """Apply the FP16 sigmoid boundary and make the single A8 payload for O."""
    if attention.shape != gate.shape or attention.ndim != 3 or attention.shape[1:] != (16, 256):
        raise ValueError("attention and gate must be [T,16,256]")
    if not all(x.is_cuda and x.is_contiguous() and x.dtype == torch.float16 for x in (attention, gate)) or gate.device != attention.device:
    ...
    if tokens:
        _gated_attention_a8_kernel[(tokens, 32)](attention, gate, data, scale, 32, BLOCK=128,
                                                   num_warps=4, num_stages=1)
    return QuantActivation("fp8", data, (tokens, 4096), scale)


@triton.jit
def _gdn_layout_kernel(source, q, k, v, src_stride: tl.constexpr, KIND: tl.constexpr, BLOCK: tl.constexpr):
    token = tl.program_id(0)
    h = tl.program_id(1)
    d = tl.arange(0, BLOCK)
    if KIND == 0:
...
def split_gdn_qkv(conv: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Materialize the GDN Q/K/V head layouts from conv[Q,K,V] directly."""
    if conv.ndim != 2 or conv.shape[1] != 8192 or conv.dtype != torch.float16 or not conv.is_cuda or not conv.is_contiguous():
    ...
    if t:
        _gdn_layout_kernel[(t, 16)](conv, q, k, v, conv.stride(0), KIND=0, BLOCK=128, num_warps=4)
        _gdn_layout_kernel[(t, 16)](conv, q, k, v, conv.stride(0), KIND=1, BLOCK=128, num_warps=4)
        _gdn_layout_kernel[(t, 32)](conv, q, k, v, conv.stride(0), KIND=2, BLOCK=128, num_warps=4)
    return q, k, v
...
def _gdn_norm_z_kernel(value, z, weight, out, heads: tl.constexpr, BLOCK: tl.constexpr):
    ...
    tl.store(out + (token * heads + head) * 128 + d,
             (norm * gate / (1.0 + tl.exp(-gate))).to(tl.float16))


@triton.jit
def _gdn_norm_z_a8_kernel(value, z, weight, q, scale, BLOCK: tl.constexpr):
    token = tl.program_id(0)
    head = tl.program_id(1)
    d = tl.arange(0, BLOCK)
    ...
    inv = tl.math.rsqrt(tl.sum(x * x, axis=0) / 128.0 + 1.0e-6)
    norm = x * inv * tl.load(weight + d).to(tl.float32)
    gate = tl.load(z + (token * 32 + head) * 128 + d).to(tl.float32)
...
def gated_gdn_fp8(value: torch.Tensor, z: torch.Tensor, norm_weight: torch.Tensor):
    ...
    if not all(x.is_cuda and x.is_contiguous() for x in (value, z, norm_weight)) or z.device != value.device or norm_weight.device != value.device:
        raise ValueError("gated GDN producer requires contiguous CUDA tensors")
    ...
    if value.shape[0]:
        _gdn_norm_z_a8_kernel[(value.shape[0], 32)](value, z, norm_weight, data, scale, BLOCK=128,
                                                     num_warps=4, num_stages=1)
    return QuantActivation("fp8", data, (value.shape[0], 4096), scale)


@triton.jit
def _gather_hidden_kernel(hidden, indices, out, rows: tl.constexpr, hidden_size: tl.constexpr,
                          SH0: tl.constexpr, SH1: tl.constexpr, SI: tl.constexpr,
                          SO0: tl.constexpr, SO1: tl.constexpr, BLOCK: tl.constexpr):
    row = tl.program_id(0)
    d = tl.arange(0, BLOCK)
    index = tl.load(indices + row * SI)
    valid = (index >= 0) & (index < rows)
    value = tl.load(hidden + index * SH0 + d * SH1, mask=valid & (d < hidden_size), other=0.0)
...
def gather_hidden(hidden: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    ...
    """
    if hidden.ndim != 2 or hidden.shape[1] != 2048 or hidden.dtype != torch.float16 or not hidden.is_cuda or not hidden.is_contiguous():
        raise ValueError("hidden must be contiguous CUDA FP16 [T,2048]")
    if indices.ndim != 1 or indices.dtype not in (torch.int32, torch.int64) or not indices.is_cuda or not indices.is_contiguous() or indices.device != hidden.device:
    ...
    if indices.numel():
        _gather_hidden_kernel[(indices.numel(),)](hidden, indices, out, hidden.shape[0], 2048,
                                                  hidden.stride(0), hidden.stride(1), indices.stride(0),
                                                  out.stride(0), out.stride(1), BLOCK=2048, num_warps=4)
    return out
...
def default_last_token_indices(cu_seqlens: torch.Tensor, tokens: int) -> torch.Tensor:
    ...
    if batch:
        _last_document_token_kernel[(batch,)](cu_seqlens, out, batch, cu_seqlens.stride(0), num_warps=1)
    return out
