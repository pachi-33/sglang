...
@triton.jit
def _qk(q, k, cu, scores, hq: tl.constexpr, hkv: tl.constexpr,
        maxs: tl.constexpr, batch: tl.constexpr, slab: tl.constexpr,
        nblocks: tl.constexpr, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    qb = tl.program_id(0)
    h = tl.program_id(1)
    bz = tl.program_id(2)
    b = bz // nblocks
    ...
    for dk in range(0, 256, BK):
        d = dk + tl.arange(0, BK)
        a = tl.load(q + tok[:, None] * hq * 256 + h * 256 + d[None, :], mask=rm[:, None] < length, other=0.0)
        # k is [T,Hkv,D], loaded transposed for TC QK
        bb = tl.load(k + ktok[None, :] * hkv * 256 + kh * 256 + d[:, None], mask=rn[None, :] < length, other=0.0)
        acc += tl.dot(a,bb)
    valid = (rm[:, None] < length) & (rn[None, :] < length) & (rn[None, :] <= rm[:, None])
    tl.store(scores + tok[:, None] * hq * 512 + h * 512 + (rn[None, :] - slab), acc * 0.0625, mask=valid)

@triton.jit
def _softmax(scores, p, local_m, local_l, cu, hq: tl.constexpr,
             batch: tl.constexpr, slab: tl.constexpr, C: tl.constexpr):
    t = tl.program_id(0)
    h = tl.program_id(1)
    start = 0
    end = 0
...
@triton.jit
def _transpose_v(v, vt, tokens: tl.constexpr, hkv: tl.constexpr):
    t = tl.program_id(0)
    hd = tl.program_id(1)
    h = hd // 16
    ...
    tl.store(
        ...
        tl.load(v + t * hkv * 256 + h * 256 + d, mask=d < 256),
        mask=d < 256,
    )

@triton.jit
def _pv(p, vt, cu, partial, hq: tl.constexpr, hkv: tl.constexpr,
        tokens: tl.constexpr, batch: tl.constexpr, slab: tl.constexpr, C: tl.constexpr):
    qb = tl.program_id(0)
    h = tl.program_id(1)
    bz = tl.program_id(2)
    b = bz // 16
    ...
    for off in range(0, C, 16):
        ...
        key = start + slab + n
        pp = tl.load(p + t[:, None] * hq * C + h * C + n[None, :], mask=(rm[:, None] < length) & (n[None, :] < C), other=0.0).to(tl.float16)
        vv = tl.load(vt + kh * 256 * tokens + d[None, :] * tokens + key[:, None], mask=(key[:, None] < end) & (d[None, :] < 256), other=0.0)
        acc += tl.dot(pp, vv)
    tl.store(partial + t[:, None] * hq * 256 + h * 256 + d[None, :], acc, mask=(rm[:, None] < length) & (d[None, :] < 256))

@triton.jit
def _merge(partial, lm, ll, m, l, acc, hq: tl.constexpr, FINAL: tl.constexpr, out, BLOCK: tl.constexpr):
    ...
    h = tl.program_id(1)
    d = tl.arange(0, BLOCK)
    oldm = tl.load(m + i * hq + h)
    oldl = tl.load(l + i * hq + h)
    ...
    newl = oldl * a + nl * b
    old = tl.load(acc + i * hq * 256 + h * 256 + d, mask=d < 256, other=0.0)
...
def causal_gqa_slab(q, k, v, cu, max_seqlen, out=None):
    ...
    m = torch.full_like(local_m, -float("inf"))
    l = torch.zeros_like(local_m)
    acc = torch.zeros((tokens, hq, 256), device=q.device, dtype=torch.float32)
    partial = torch.empty_like(acc)
    if out is None:
        out = torch.empty_like(q)
    _transpose_v[(tokens, 32)](v, vt, tokens, 2, num_warps=4)
    for slab in range(0, max_seqlen, columns):
        nblocks = triton.cdiv(min(columns, max_seqlen - slab), 16)
        _qk[(triton.cdiv(max_seqlen, 16), hq, batch * nblocks)](q, k, cu, scores, hq, 2, max_seqlen, batch, slab, nblocks, BM=16, BN=16, BK=16, num_warps=4, num_stages=1)
        _softmax[(tokens, hq)](scores, p, local_m, local_l, cu, hq, batch, slab, columns, num_warps=4)
        _pv[(triton.cdiv(max_seqlen, 16), hq, batch * 16)](p, vt, cu, partial, hq, 2, tokens, batch, slab, columns, num_warps=4, num_stages=1)
        _merge[(tokens, hq)](partial, local_m, local_l, m, l, acc, hq, slab + columns >= max_seqlen, out, BLOCK=256, num_warps=4)
    return out
