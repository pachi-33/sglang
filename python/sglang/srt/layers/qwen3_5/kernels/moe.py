...
@triton.jit
def dispatch_count_kernel(ids_ptr, counts_ptr, routes: tl.constexpr, ROUTE_BLOCK: tl.constexpr):
...
@triton.jit
def dispatch_init_kernel(source_ptr, positions_ptr, capacity: tl.constexpr, BLOCK: tl.constexpr):
...
@triton.jit
def dispatch_block_expert_kernel(offsets_ptr, block_expert_ptr, max_blocks: tl.constexpr):
    ...
    tl.store(block_expert_ptr + block, tl.where(selected == 256, -1, selected), mask=block < max_blocks)


@triton.jit
def dispatch_stable_scatter_kernel(
    ids_ptr, source_ptr, positions_ptr, offsets_ptr, routes: tl.constexpr, ROUTE_BLOCK: tl.constexpr
):
    route = tl.program_id(0)
    expert = tl.load(ids_ptr + route, mask=route < routes, other=-1)
    prior = tl.arange(0, ROUTE_BLOCK)
    all_ids = tl.load(ids_ptr + prior, mask=prior < routes, other=-2)
    rank = tl.sum(((prior < route) & (all_ids == expert)).to(tl.int32), axis=0)
    destination = tl.load(offsets_ptr + expert, mask=(route < routes) & (expert >= 0), other=0) * 32 + rank
    tl.store(source_ptr + destination, route // 8, mask=route < routes)
    tl.store(positions_ptr + destination, route, mask=route < routes)


@triton.jit
def dispatch_inverse_kernel(positions_ptr, inverse_ptr, rows: tl.constexpr, routes: tl.constexpr, BLOCK: tl.constexpr):
    row = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    position = tl.load(positions_ptr + row, mask=row < rows, other=-1)
    tl.store(inverse_ptr + position, row, mask=(row < rows) & (position >= 0) & (position < routes))


@triton.jit
def route_combine_kernel(
    down_ptr, inverse_ptr, weights_ptr, out_ptr,
    tokens: tl.constexpr, hidden: tl.constexpr, stride_down_m: tl.constexpr, BLOCK: tl.constexpr,
):
    ...
    for slot in range(0, 8):
        ...
        value = tl.load(down_ptr + safe_row * stride_down_m + cols, mask=(row >= 0) & (cols < hidden), other=0.0)
        weight = tl.load(weights_ptr + token * 8 + slot, mask=token < tokens, other=0.0)
        acc += value.to(tl.float32) * weight
    tl.store(out_ptr + token * hidden + cols, acc, mask=(token < tokens) & (cols < hidden))


@triton.jit
def normalized_top8_kernel(
    logits_ptr, ids_ptr, raw_weights_ptr, tokens: tl.constexpr, experts: tl.constexpr, TOPK: tl.constexpr
):
    """One CTA per token: stable FP32 softmax and exact lower-ID tie breaks."""
    token = tl.program_id(0)
    expert = tl.arange(0, 256)
    logits = tl.load(logits_ptr + token * experts + expert, mask=(token < tokens) & (expert < experts), other=-float("inf")).to(tl.float32)
...
@triton.jit
def normalize_topk_weights_kernel(weights_ptr, tokens: tl.constexpr, TOPK: tl.constexpr):
    token = tl.program_id(0)
    slot = tl.arange(0, TOPK)
    weights = tl.load(weights_ptr + token * TOPK + slot, mask=token < tokens, other=0.0).to(tl.float32)
    ...
    value = tl.where(
        ...
        tl.where(
            ...
            tl.where(
                mag == 2,
                1.0,
                tl.where(mag == 3, 1.5, tl.where(mag == 4, 2.0, tl.where(mag == 5, 3.0, tl.where(mag == 6, 4.0, 6.0)))),
            ),
        ),
    )
...
def _e4m3fn(byte):
    ...
    normal = (1.0 + mantissa.to(tl.float32) * 0.125) * tl.math.exp2(exponent.to(tl.float32) - 7.0)
...
def nvfp4_grouped_gemm_kernel(
    ...
    raw_expert = tl.load(expert_of_block_ptr + pid_m, mask=pid_m < (rows // BM), other=-1)
    ...
    if valid_expert:
        ...
        for k0 in range(0, k, BK):
            ...
            offs_k = packed_k * 2
            a_byte = tl.load(a_ptr + offs_m[:, None] * stride_am + packed_k[None, :], mask=(offs_m[:, None] < rows) & (offs_k[None, :] < k), other=0)
            a_lo_code, a_hi_code = a_byte & 15, a_byte >> 4
            w_byte = tl.load(w_ptr + expert * stride_we + offs_n[None, :] * stride_wn + packed_k[:, None], mask=(offs_n[None, :] < n) & (offs_k[:, None] < k), other=0)
            w_lo_code, w_hi_code = w_byte & 15, w_byte >> 4
            a_sf = tl.load(a_sf_ptr + offs_m[:, None] * stride_as_m + (offs_k[None, :] // 16), mask=(offs_m[:, None] < rows) & (offs_k[None, :] < k), other=0)
            w_sf = tl.load(w_sf_ptr + expert * stride_ws_e + offs_n[None, :] * stride_ws_n + (offs_k[:, None] // 16), mask=(offs_n[None, :] < n) & (offs_k[:, None] < k), other=0)
        ...
        out = acc * (1.0 / (tl.load(a_global_ptr + expert) * tl.load(w_global_ptr + expert)))
    else:
    ...
    if WEIGHTED:
        ...
        value = out
    tl.store(out_ptr + offs_m[:, None] * stride_om + offs_n[None, :] * stride_on, value, mask=store_mask)


@triton.jit
def route_combine_weighted_kernel(
    down_ptr, inverse_ptr, out_ptr,
    tokens: tl.constexpr, hidden: tl.constexpr, stride_down_m: tl.constexpr, BLOCK: tl.constexpr,
):
    token = tl.program_id(0)
    cols = tl.program_id(1) * BLOCK + tl.arange(0, BLOCK)
    ...
    for slot in range(0, 8):
        row = tl.load(inverse_ptr + token * 8 + slot, mask=token < tokens, other=-1)
        acc += tl.load(down_ptr + tl.maximum(row, 0) * stride_down_m + cols, mask=(row >= 0) & (cols < hidden), other=0.0)
    tl.store(out_ptr + token * hidden + cols, acc, mask=(token < tokens) & (cols < hidden))
    ...
    for work in range(0, MAX_TILES_PER_PROGRAM):
        ...
        if valid_expert:
            ...
            for k0 in range(0, k, 32):
                packed_k = k0 // 2 + tl.arange(0, 16)
                offs_k = packed_k * 2
                a_byte = tl.load(a_ptr + safe_source[:, None] * stride_am + packed_k[None, :], mask=valid_source[:, None] & (offs_k[None, :] < k), other=0)
                a_sf = tl.load(a_sf_ptr + safe_source[:, None] * stride_as_m + (offs_k[None, :] // 16), mask=valid_source[:, None] & (offs_k[None, :] < k), other=0)
                a_lo = (_e2m1(a_byte & 15) * _e4m3fn(a_sf)).to(tl.float16)
                a_hi = (_e2m1(a_byte >> 4) * _e4m3fn(a_sf)).to(tl.float16)
                gate_byte = tl.load(w_ptr + expert * stride_we + offs_n[None, :] * stride_wn + packed_k[:, None], mask=offs_k[:, None] < k, other=0)
                up_byte = tl.load(w_ptr + expert * stride_we + (offs_n[None, :] + intermediate) * stride_wn + packed_k[:, None], mask=offs_k[:, None] < k, other=0)
                gate_sf = tl.load(w_sf_ptr + expert * stride_ws_e + offs_n[None, :] * stride_ws_n + (offs_k[:, None] // 16), mask=offs_k[:, None] < k, other=0)
                up_sf = tl.load(w_sf_ptr + expert * stride_ws_e + (offs_n[None, :] + intermediate) * stride_ws_n + (offs_k[:, None] // 16), mask=offs_k[:, None] < k, other=0)
            ...
            inv_global = 1.0 / (tl.load(gate_a_global_ptr + expert).to(tl.float32) * tl.load(gate_w_global_ptr + expert).to(tl.float32))
            gate = (gate_acc * inv_global).to(tl.float16)
            up = (up_acc * inv_global).to(tl.float16)
            z = (gate.to(tl.float32) * tl.sigmoid(gate.to(tl.float32)) * up.to(tl.float32)).to(tl.float16)
        else:
            z = tl.zeros((32, 32), tl.float16)
        tl.store(scratch + rows32[:, None] * 32 + cols32[None, :], z)
        if CAPTURE_Z:
            tl.store(capture_z_ptr + offs_m[:, None] * intermediate + offs_n[None, :], z, mask=valid_expert)
        tl.debug_barrier()
        ...
        for group_in_tile in range(0, 2):
            ...
            z_lo = tl.load(scratch + rows32[:, None] * 32 + local[None, :], volatile=True).to(tl.float32)
            z_hi = tl.load(scratch + rows32[:, None] * 32 + local[None, :] + 1, volatile=True).to(tl.float32)
            down_g = tl.load(down_a_global_ptr + expert).to(tl.float32)
            u_lo = z_lo * down_g
            u_hi = z_hi * down_g
            raw_sf = tl.maximum(tl.max(tl.abs(u_lo), axis=1), tl.max(tl.abs(u_hi), axis=1)) / 6.0
            ...
            tl.store(
                z_ptr + offs_m[:, None] * (intermediate // 2) + z_group * 8 + pair[None, :],
                packed,
                mask=valid_expert,
            )
            tl.store(
...
def fp16_grouped_gemm_kernel(
    a_ptr, source_ids_ptr, expert_of_block_ptr, w_ptr, out_ptr,
    rows: tl.constexpr, n: tl.constexpr, k: tl.constexpr,
    stride_am: tl.constexpr, stride_we: tl.constexpr, stride_wn: tl.constexpr,
    ...
    for k0 in range(0, k, 32):
        ...
        a = tl.load(a_ptr + safe_source[:, None] * stride_am + offs_k[None, :], mask=valid_expert & valid_source[:, None] & (offs_k[None, :] < k), other=0.0)
        w = tl.load(w_ptr + expert * stride_we + offs_n[None, :] * stride_wn + offs_k[:, None], mask=valid_expert & (offs_n[None, :] < n) & (offs_k[:, None] < k), other=0.0)
        acc += tl.dot(a, w)
    tl.store(out_ptr + offs_m[:, None] * stride_om + offs_n[None, :] * stride_on, acc, mask=valid_expert & (offs_n[None, :] < n))


@triton.jit
def fp16_swiglu_kernel(gate_up_ptr, z_ptr, rows: tl.constexpr, intermediate: tl.constexpr, stride_gu_m: tl.constexpr, stride_z_m: tl.constexpr):
    row = tl.program_id(0)
    cols = tl.program_id(1) * 32 + tl.arange(0, 32)
    gate = tl.load(gate_up_ptr + row * stride_gu_m + cols, mask=(row < rows) & (cols < intermediate), other=0.0)
    up = tl.load(gate_up_ptr + row * stride_gu_m + intermediate + cols, mask=(row < rows) & (cols < intermediate), other=0.0)
    z = (gate.to(tl.float32) * tl.sigmoid(gate.to(tl.float32)) * up.to(tl.float32)).to(tl.float16)
    tl.store(z_ptr + row * stride_z_m + cols, z, mask=(row < rows) & (cols < intermediate))


@triton.jit
def fp16_sigmoid_multiply_kernel(value_ptr, gate_ptr, out_ptr, rows: tl.constexpr, hidden: tl.constexpr, stride_value_m: tl.constexpr, stride_gate_m: tl.constexpr, stride_out_m: tl.constexpr, GATE_SCALAR: tl.constexpr, BLOCK: tl.constexpr):
    row = tl.program_id(0)
    cols = tl.program_id(1) * BLOCK + tl.arange(0, BLOCK)
    value = tl.load(value_ptr + row * stride_value_m + cols, mask=(row < rows) & (cols < hidden), other=0.0)
    gate = tl.load(gate_ptr + row * stride_gate_m + tl.where(GATE_SCALAR, 0, cols), mask=(row < rows) & (cols < hidden), other=0.0)
    ...
    tl.store(out_ptr + row * stride_out_m + cols, out, mask=(row < rows) & (cols < hidden))


@triton.jit
def fp16_add_kernel(a_ptr, b_ptr, out_ptr, rows: tl.constexpr, hidden: tl.constexpr, stride_a_m: tl.constexpr, stride_b_m: tl.constexpr, stride_out_m: tl.constexpr, BLOCK: tl.constexpr):
    row = tl.program_id(0)
    cols = tl.program_id(1) * BLOCK + tl.arange(0, BLOCK)
    a = tl.load(a_ptr + row * stride_a_m + cols, mask=(row < rows) & (cols < hidden), other=0.0)
    b = tl.load(b_ptr + row * stride_b_m + cols, mask=(row < rows) & (cols < hidden), other=0.0)
    tl.store(out_ptr + row * stride_out_m + cols, (a.to(tl.float32) + b.to(tl.float32)).to(tl.float16), mask=(row < rows) & (cols < hidden))
