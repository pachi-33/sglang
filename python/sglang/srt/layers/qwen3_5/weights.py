...
class Weight:
    ...
    def __post_init__(self) -> None:
        ...
        if any(dim <= 0 for dim in self.logical_shape):
            raise ValueError(
                f"logical shape dimensions must be positive: {self.logical_shape}"
            )
        ...
        if self.kind == "fp16":
            ...
            if any(
                x is not None
                for x in (self.block_scale, self.global_scale, self.input_global_scale)
            ):
            ...
            if tuple(self.block_scale.shape) != expected_scale:
                raise ValueError(
                    f"fp8 scale shape {tuple(self.block_scale.shape)} != {expected_scale}"
                )
            ...
            if self.data.dtype != torch.uint8 or self.block_scale is None:
                raise TypeError(
                    "nvfp4 weights require uint8 packed data and block_scale"
                )
            expected_scale = prefix + (n, (k + 15) // 16)
            if tuple(self.block_scale.shape) != expected_scale:
                raise ValueError(
                    f"nvfp4 scale shape {tuple(self.block_scale.shape)} != {expected_scale}"
                )
            ...
            for scale, name in (
                ...
                if scale.dtype != torch.float32 or scale.numel() not in (
                    1,
                    expert_count,
                ):
...
class QuantActivation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "fp8":
            ...
            if (
                k % 2
                or tuple(self.data.shape) != (m, k // 2)
            ...
            if (
                self.block_scale.dtype != torch.uint8
                or self.global_scale.dtype != torch.float32
            ):
                raise TypeError(
                    "nvfp4 activation requires uint8 scale and float32 global scale"
                )
            if self.global_scale.numel() not in (1, m):
                raise ValueError(
                    "nvfp4 activation global scale must be scalar/[1] or [M]"
                )
        else:
            raise ValueError(f"unsupported activation kind: {self.kind}")
        if self.data.device != self.block_scale.device or (
            self.global_scale is not None
            and self.data.device != self.global_scale.device
        ):
