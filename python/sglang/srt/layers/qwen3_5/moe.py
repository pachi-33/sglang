...
def _validate_nvfp4_expert_call(x: torch.Tensor, gate_up: Any, down: Any, ids: torch.Tensor, weights: torch.Tensor) -> None:
    ...
    if ids.dtype != torch.int32 or weights.dtype != torch.float32 or not ids.is_cuda or not weights.is_cuda:
        raise ValueError("ids must be CUDA int32 and route weights CUDA float32")
    if not ids.is_contiguous() or not weights.is_contiguous() or ids.shape != (x.shape[0], 8) or weights.shape != ids.shape:
    ...
    if x.shape[1] <= 0 or x.shape[1] % 32:
        raise ValueError("NVFP4 routed hidden size must be positive and divisible by 32")
    ...
    if gate_data.ndim != 3 or down_data.ndim != 3 or gate_data.shape[0] != 256 or down_data.shape[0] != 256:
    ...
    if intermediate <= 0 or intermediate % 32:
        raise ValueError("NVFP4 routed intermediate size must be positive and divisible by 32")
    if gate_data.shape != (256, intermediate * 2, x.shape[1] // 2) or down_data.shape != (256, x.shape[1], intermediate // 2):
        raise ValueError("NVFP4 routed shapes must be gate/up [256,2I,H], down [256,H,I]")
    expected_gate_sf = (256, intermediate * 2, x.shape[1] // 16)
    expected_down_sf = (256, x.shape[1], intermediate // 16)
    tensors = (
        (gate_data, torch.uint8, "gate/up data"), (gate_sf, torch.uint8, "gate/up scales"),
        (down_data, torch.uint8, "down data"), (down_sf, torch.uint8, "down scales"),
    ...
    if gate_sf.shape != expected_gate_sf or down_sf.shape != expected_down_sf:
        raise ValueError("NVFP4 scale payload shape is inconsistent with routed matrices")
    for tensor, dtype, label in tensors:
        if tensor.dtype != dtype or not tensor.is_cuda or tensor.device != x.device or not tensor.is_contiguous():
...
def _validate_fp16_expert_call(x: torch.Tensor, gate_up: Any, down: Any, ids: torch.Tensor, weights: torch.Tensor) -> None:
    """Validate FP16 routed tensors before grouped-GEMM indexing."""
    if x.ndim != 2 or x.dtype != torch.float16 or not x.is_cuda or not x.is_contiguous():
        raise ValueError("x must be contiguous CUDA FP16 [T,H]")
    if ids.dtype != torch.int32 or weights.dtype != torch.float32 or not ids.is_cuda or not weights.is_cuda:
        raise ValueError("ids must be CUDA int32 and route weights CUDA float32")
    if ids.shape != (x.shape[0], 8) or weights.shape != ids.shape or not ids.is_contiguous() or not weights.is_contiguous():
        raise ValueError("ids and route weights must be contiguous [T,8]")
    if ids.device != x.device or weights.device != x.device:
        raise ValueError("x, ids, and route weights must share a CUDA device")
    ...
    intermediate = gate_data.shape[1] // 2
    if intermediate <= 0 or gate_data.shape != (256, intermediate * 2, x.shape[1]) or down_data.shape != (256, x.shape[1], intermediate):
        raise ValueError("FP16 routed shapes must be gate/up [256,2I,H], down [256,H,I]")
    for tensor, label in ((gate_data, "gate/up data"), (down_data, "down data")):
        if tensor.dtype != torch.float16 or not tensor.is_cuda or tensor.device != x.device or not tensor.is_contiguous():
            raise ValueError(f"{label} must be contiguous CUDA FP16 on x.device")


def route_topk(logits: torch.Tensor, top_k: int = 8) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if not logits.is_cuda or not logits.is_contiguous() or logits.shape[1] != 256 or top_k != 8:
    ...
    if tokens:
        normalized_top8_kernel[(tokens,)](logits, ids, weights, tokens=tokens, experts=256, TOPK=8, num_warps=4)
        normalize_topk_weights_kernel[(tokens,)](weights, tokens=tokens, TOPK=8, num_warps=1)
    return ids, weights


def _build_dispatch(ids: torch.Tensor, num_experts: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    dispatch_init_kernel[(triton.cdiv(capacity, 256),)](source, positions, capacity=capacity, BLOCK=256)
    route_block = triton.next_power_of_2(max(routes, 1))
    dispatch_count_kernel[(256,)](ids, counts, routes=routes, ROUTE_BLOCK=route_block, num_warps=4)
    dispatch_prefix_kernel[(1,)](counts, offsets, num_warps=1)
    dispatch_block_expert_kernel[(max_blocks,)](offsets, expert_blocks, max_blocks=max_blocks, num_warps=1)
    if routes:
        dispatch_stable_scatter_kernel[(routes,)](
            ids, source, positions, offsets, routes=routes, ROUTE_BLOCK=route_block, num_warps=4
        )
...
def _grouped_gemm(
    ...
    *,
    ...
    if data.dtype != torch.uint8 or scale.dtype != torch.uint8:
        raise TypeError("NVFP4 grouped GEMM needs raw uint8 packed data and E4M3 scale bytes")
    ...
    if ag.numel() != num_experts:
        raise ValueError("V100 grouped GEMM requires physical per-expert activation globals")
    if weight_global.numel() != num_experts:
        raise ValueError("V100 grouped GEMM requires physical per-expert weight globals")
    ...
    if expert_major:
        if tuple(activation.data.shape) != (rows, k // 2) or tuple(activation.block_scale.shape) != (rows, k // 16):
        ...
        gathered_data = torch.empty((rows, half_k), dtype=torch.uint8, device=data.device)
        gathered_sf = torch.empty((rows, sf_k), dtype=torch.uint8, device=data.device)
        gather_nvfp4_activation_kernel[(rows,)](
            activation.data, activation.block_scale, source_ids, gathered_data, gathered_sf,
            rows=rows, half_k=half_k, sf_k=sf_k,
            stride_src_m=activation.data.stride(0), stride_src_sf_m=activation.block_scale.stride(0),
            BLOCK=triton.next_power_of_2(half_k), num_warps=4,
        )
    out = torch.empty((rows, n), dtype=torch.float32 if weighted else torch.float16, device=data.device)
...
def _paired_gemm1_swiglu_a4(
    ...
    *,
    ...
    if outputs != intermediate * 2 or intermediate % 32 or k % 32 or rows % 32:
        raise ValueError("paired V100 GEMM1 requires [E,2I,K] with I/K/rows divisible by 32")
    ...
    if rows == 0:
        ...
        return (payload, torch.empty((0, intermediate), dtype=torch.float16, device=data.device)) if capture_z else payload

    z_data = torch.empty((rows, intermediate // 2), dtype=torch.uint8, device=data.device)
    z_sf = torch.empty((rows, intermediate // 16), dtype=torch.uint8, device=data.device)
    ...
    programs = min(tiles, 80)
    scratch = torch.empty((programs, 32, 32), dtype=torch.float16, device=data.device)
    z_capture = torch.empty((rows, intermediate), dtype=torch.float16, device=data.device) if capture_z else scratch
    ...
    payload = ExpertActivation(z_data, (rows, intermediate), z_sf, expert_blocks, down_a_global)
...
def execute_experts(
    ...
    down_out = _grouped_gemm(
        qz, None, expert_blocks, down, down_ga,
        positions=positions, route_weights=weights, expert_major=True,
    )
    ...
    route_combine_weighted_kernel[(x.shape[0], triton.cdiv(x.shape[1], 256))](
        down_out, inverse, out,
        tokens=x.shape[0], hidden=x.shape[1], stride_down_m=down_out.stride(0), BLOCK=256,
        num_warps=4,
    )
    return out
...
def _fp16_swiglu_values(gate_up: torch.Tensor) -> torch.Tensor:
    ...
    if rows:
        fp16_swiglu_kernel[(rows, triton.cdiv(intermediate, 32))](
            gate_up, z, rows=rows, intermediate=intermediate,
            stride_gu_m=gate_up.stride(0), stride_z_m=z.stride(0), num_warps=1,
        )
    return z


def _fp16_sigmoid_multiply(value: torch.Tensor, gate: torch.Tensor) -> torch.Tensor:
    if gate.ndim != 2 or gate.shape[0] != value.shape[0] or gate.shape[1] not in (1, value.shape[1]):
    ...
    if rows:
        fp16_sigmoid_multiply_kernel[(rows, triton.cdiv(hidden, 256))](
            value, gate, out, rows=rows, hidden=hidden,
            stride_value_m=value.stride(0), stride_gate_m=gate.stride(0), stride_out_m=out.stride(0),
            GATE_SCALAR=gate.shape[1] == 1, BLOCK=256, num_warps=4,
        )
    return out
...
def _fp16_add(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    if rows:
        fp16_add_kernel[(rows, triton.cdiv(hidden, 256))](
            a, b, out, rows=rows, hidden=hidden,
            stride_a_m=a.stride(0), stride_b_m=b.stride(0), stride_out_m=out.stride(0),
...
def _fp16_grouped_gemm(
    ...
    fp16_grouped_gemm_kernel[(rows // 32, triton.cdiv(n, 32))](
        activation, source_ids, expert_blocks, data, out,
        rows=rows, n=n, k=k,
        stride_am=activation.stride(0), stride_we=data.stride(0), stride_wn=data.stride(1),
        stride_om=out.stride(0), stride_on=out.stride(1), num_warps=4, num_stages=1,
    )
    return out
...
def execute_fp16_experts(
    ...
    row_ids = torch.arange(source.numel(), device=x.device, dtype=torch.int32)
    down_out = _fp16_grouped_gemm(z, row_ids, expert_blocks, down)
    inverse = torch.empty((ids.numel(),), device=x.device, dtype=torch.int32)
    dispatch_inverse_kernel[(triton.cdiv(positions.numel(), 256),)](
        positions, inverse, rows=positions.numel(), routes=ids.numel(), BLOCK=256
    )
...
def fused_nvfp4_moe(
    ...
    if getattr(weights.gate_up, "kind", None) == "fp16":
        ...
        out = execute_experts(x, weights.gate_up, weights.down, ids, probs)
    if weights.shared_gate_up is not None and weights.shared_down is not None:
        ...
        if weights.shared_gate is not None:
            shared = _fp16_sigmoid_multiply(shared, _fp16_linear(x, weights.shared_gate))
        out = _fp16_add(out, shared.to(out.dtype))
    return out
