...
class TestStatelessFourLayerSlice(V100TestCase):
    ...
    def _forward(self, lengths, *, logits_indices=None, start_id=1):
        ...
        return self.model.forward_no_cache(
            input_ids=ids, positions=positions, cu_seqlens=cu,
            max_seqlen=max(lengths, default=0), logits_indices=logits_indices,
        )

    @staticmethod
    def _nrmse(actual, expected):
        return ((actual.float() - expected.float()).square().mean().sqrt() /
                expected.float().square().mean().sqrt().clamp_min(1e-8)).item()
    ...
    def test_real_chain_length_matrix_and_empty_batch(self):
        ...
        self.assertEqual((tuple(hidden.shape), tuple(logits.shape)), ((87, 2048), (4, 248320)))
        self.assertTrue(torch.isfinite(hidden).all() and torch.isfinite(logits).all())
        ids, positions, cu = self._packed([0])
        hidden, logits = self.model.forward_no_cache(
            input_ids=ids, positions=positions, cu_seqlens=cu, max_seqlen=0,
        )
        self.assertEqual((tuple(hidden.shape), tuple(logits.shape)), ((0, 2048), (0, 248320)))

    def test_default_empty_logits_and_explicit_selection(self):
        hidden, default_logits = self._forward([0, 1, 0, 2], start_id=500)
        self.assertEqual((tuple(hidden.shape), tuple(default_logits.shape)), ((3, 2048), (4, 248320)))
        torch.testing.assert_close(default_logits[0], torch.zeros_like(default_logits[0]), rtol=0, atol=0)
        torch.testing.assert_close(default_logits[2], torch.zeros_like(default_logits[2]), rtol=0, atol=0)
        explicit = torch.tensor([0, 2], device="cuda", dtype=torch.int32)
        _, explicit_logits = self._forward([0, 1, 0, 2], logits_indices=explicit, start_id=500)
        torch.testing.assert_close(explicit_logits, default_logits[[1, 3]], rtol=0, atol=0)
    ...
    def test_input_or_hidden_exclusive_and_packed_sequence_equivalence(self):
        ...
        with self.assertRaises(ValueError):
            self.model.forward_no_cache(
                input_ids=ids, hidden_states=embedded, positions=positions,
                cu_seqlens=cu, max_seqlen=65,
            )
        packed_hidden, packed_logits = self.model.forward_no_cache(
            hidden_states=embedded, positions=positions, cu_seqlens=cu, max_seqlen=65,
        )
        ...
        for length in (17, 65):
            ...
            local_hidden, local_logits = self.model.forward_no_cache(
                ...
                cu_seqlens=torch.tensor([0, length], device="cuda", dtype=torch.int32), max_seqlen=length,
            )
        ...
        self._report(case="packed_vs_per_sequence", hidden_nrmse=hidden_nrmse, logits_nrmse=logits_nrmse,
    ...
    def test_repeat_after_distinct_call_and_independent_global_oracles(self):
        ...
        torch.testing.assert_close(embedded, runner.global_weights["embed_tokens"].data[ids.long()], rtol=0, atol=0)
        before_final = runner.forward_hidden(embedded, positions=positions, cu_seqlens=cu, max_seqlen=1)
        expected_final = (before_final.float() * torch.rsqrt(before_final.float().square().mean(-1, keepdim=True) + 1e-6) *
                          (1 + runner.global_weights["final_norm"].float())).half()
        ...
        torch.testing.assert_close(actual_logits[:, :4096], expected_block, rtol=2e-3, atol=2e-3)
    ...
    def test_short_sequences_ignore_an_inflated_batch_maximum(self):
        ...
        reference_hidden, reference_logits = self.model.forward_no_cache(
            input_ids=ids, positions=positions, cu_seqlens=cu, max_seqlen=17,
        )
        for declared_max in (65, 2048):
            with self.subTest(declared_max=declared_max):
                hidden, logits = self.model.forward_no_cache(
                    input_ids=ids, positions=positions, cu_seqlens=cu, max_seqlen=declared_max,
                )
    ...
    def test_int64_empty_and_63_64_65_sequence_isolation(self):
        ...
        packed_hidden, packed_logits = self.model.forward_no_cache(
            input_ids=ids, positions=positions, cu_seqlens=cu, max_seqlen=65,
        ...
        for length in (63, 64, 65):
            ...
            separate_logits.append(local_logits)
            cursor = end
        torch.testing.assert_close(packed_hidden, torch.cat(separate_hidden), rtol=0, atol=0)
        torch.testing.assert_close(packed_logits, torch.cat(separate_logits), rtol=0, atol=0)

    def test_single_sequence_t2048_peak(self):
        self._forward([1], logits_indices=torch.tensor([0], device="cuda", dtype=torch.int32), start_id=1450)
        torch.cuda.synchronize()
        torch.cuda.reset_peak_memory_stats()
        hidden, logits = self._forward([2048], start_id=1600)
        torch.cuda.synchronize()
        peak = torch.cuda.max_memory_allocated()
        self._report(case="single_sequence_t2048", peak_bytes=peak, shape=list(hidden.shape),
                     finite=bool(torch.isfinite(hidden).all() and torch.isfinite(logits).all()))
        self.assertEqual((tuple(hidden.shape), tuple(logits.shape)), ((2048, 2048), (1, 248320)))
        self.assertTrue(torch.isfinite(hidden).all() and torch.isfinite(logits).all())
    ...
    def test_worst_ragged_t2048_peak(self):
        ...
        self._forward([1], logits_indices=torch.tensor([0], device="cuda", dtype=torch.int32), start_id=1400)
        torch.cuda.synchronize()
        torch.cuda.reset_peak_memory_stats()
        hidden, logits = self._forward(lengths, logits_indices=indices, start_id=1500)
        torch.cuda.synchronize()
        peak = torch.cuda.max_memory_allocated()
        self._report(case="worst_ragged_t2048", peak_bytes=peak, shape=list(hidden.shape), finite=bool(torch.isfinite(hidden).all() and torch.isfinite(logits).all()))
        self.assertEqual((tuple(hidden.shape), tuple(logits.shape)), ((2048, 2048), (1, 248320)))
        self.assertTrue(torch.isfinite(hidden).all() and torch.isfinite(logits).all())
        self.assertLessEqual(peak, 12 * 1024 ** 3)
...
class TestZZLayer39FullAttention(V100TestCase):
    ...
    def setUpClass(cls):
        ...
        if not MODEL_DIR.is_dir():
            raise unittest.SkipTest("real checkpoint unavailable")
        cls.runner = Qwen35StatelessRunner(MODEL_DIR, layer_ids=[39], load_globals=False)
    ...
    def test_layer39_full_attention_fp16_expert_route(self):
        ...
        result = self.runner.forward_layer(
            ...
            cu_seqlens=torch.tensor([0, 3], device="cuda", dtype=torch.int32), max_seqlen=3,
        )
        torch.cuda.synchronize()
