...
def recurrent(q, k, v, decay, beta, cu):
    out = torch.empty((q.shape[0], v.shape[1], 128), device=q.device, dtype=torch.float32)
    states = []
...
def recurrent_vectorized(q, k, v, decay, beta, cu):
    ...
    for start, end in zip(cu[:-1].tolist(), cu[1:].tolist()):
        ...
        for t in range(start, end):
            ...
            residual = beta[t].float().view(32, 1) * (v[t].float() - torch.bmm(state, kt.unsqueeze(2)).squeeze(2))
