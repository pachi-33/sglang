...
from sglang.srt.layers.qwen3_5.ops import (
    gated_rms_norm_silu,
    gemma_rms_norm,
    residual_add,
    sigmoid_mul,
...
class TestDenseOps(V100TestCase):
    def test_nt_gemm_sm70_shape_sweep(self):
        ...
        self.assertEqual(
            tuple(linear_fp16(empty, Weight("fp16", weight, (64, 128))).shape), (0, 64)
        )
    ...
    def test_nt_gemm_and_embedding(self):
        ...
        with self.assertRaises(IndexError):
            fp16_embedding(
                torch.tensor([31], dtype=torch.int64, device="cuda"),
                table,
                validate_ids=True,
            )
    ...
    def test_norm_and_elementwise(self):
        ...
        torch.testing.assert_close(
            gemma_rms_norm(x, weight), ref_gemma(x, weight), rtol=3e-3, atol=5e-3
        )
        torch.testing.assert_close(
            gated_rms_norm_silu(x, y, weight),
            ref_gated(x, y, weight),
            rtol=3e-3,
            atol=5e-3,
        )
        torch.testing.assert_close(
            residual_add(x, y), (x + y).half(), rtol=1e-3, atol=1e-3
        )
        torch.testing.assert_close(
            sigmoid_mul(x, y), (x * y.sigmoid()).half(), rtol=3e-3, atol=2e-3
        )
        ...
        with self.assertRaises(ValueError):
            linear_fp16(
                ...
                Weight(
                    "fp16",
                    torch.randn((64, 1024), dtype=torch.float16, device="cuda"),
                    (64, 1024),
                ),
            )
