...
REAL_NVFP4_CHECKPOINT = Path("/home/yaozhenyang/huggingface/Qwen-AgentWorld-35B-A3B-NVFP4_fp16")
...
class TestQwen35Moe(V100TestCase):
    def test_topk_is_fp32_and_sorted(self):
        ...
        logits[0, :4] = torch.tensor([0.0, 3.0, 2.0, 1.0], dtype=torch.float16, device="cuda")
        logits[1] = 0.0
    ...
    def test_fp16_routed_moe_fallback(self):
        ...
        gate_data[0] = torch.tensor([[1.0, 0.0], [0.0, 1.0], [2.0, 0.0], [0.0, 2.0]], dtype=torch.float16, device="cuda")
        ...
        out = execute_fp16_experts(x, gate_up, down, torch.zeros((1, 8), dtype=torch.int32, device="cuda"), torch.full((1, 8), 0.125, dtype=torch.float32, device="cuda"))
        expected = (
            torch.nn.functional.silu(torch.tensor([[1.0, -1.0]], dtype=torch.float16, device="cuda").float())
    ...
    def test_routed_validators_reject_malformed_payloads_before_launch(self):
        ...
        fp_gate = types.SimpleNamespace(data=torch.zeros((256, 4, 2), dtype=torch.float16, device="cuda"))
        fp_down = types.SimpleNamespace(data=torch.zeros((256, 2, 2), dtype=torch.float16, device="cuda"))
        x2 = torch.zeros((1, 2), dtype=torch.float16, device="cuda")
        with self.assertRaises(ValueError):
            execute_fp16_experts(torch.zeros((1, 4), dtype=torch.float16, device="cuda")[:, ::2], fp_gate, fp_down, ids, route_weights)
        with self.assertRaises(ValueError):
            execute_fp16_experts(x2, fp_gate, types.SimpleNamespace(data=torch.zeros((256, 1, 2), dtype=torch.float16, device="cuda")), ids, route_weights)
        with self.assertRaises(ValueError):
            execute_fp16_experts(x2, fp_gate, fp_down, ids, route_weights.half())
        noncontig_gate = types.SimpleNamespace(data=torch.zeros((256, 4, 4), dtype=torch.float16, device="cuda")[:, :, ::2])
        with self.assertRaises(ValueError):
        ...
        nv_gate, nv_down = _nvfp4_weight(nv_gate_cpu, device="cuda"), _nvfp4_weight(nv_down_cpu, device="cuda")
        with self.assertRaises(ValueError):
            execute_experts(torch.zeros((1, 30), dtype=torch.float16, device="cuda"), nv_gate, nv_down, ids, route_weights)
        with self.assertRaises(ValueError):
            execute_experts(torch.zeros((1, 64), dtype=torch.float16, device="cuda")[:, ::2], nv_gate, nv_down, ids, route_weights)
        with self.assertRaises(ValueError):
            execute_experts(torch.zeros((1, 32), dtype=torch.float16, device="cuda"), nv_gate, nv_down, ids, torch.full((1, 16), 0.125, dtype=torch.float32, device="cuda")[:, ::2])
...
class TestQwen35MoeGPU(V100TestCase):
    def test_gpu_dispatch_matches_stable_reference(self):
        for tokens, hotspot in ((0, False), (1, False), (17, False), (32, True), (128, False)):
            with self.subTest(tokens=tokens, hotspot=hotspot):
                routes = tokens * 8
                flat = (torch.zeros((routes,), dtype=torch.int32) if hotspot else torch.arange(routes, dtype=torch.int32) % 256)
                ...
                expected_positions = torch.full((max_blocks * 32,), -1, dtype=torch.int32)
                ...
                for expert in range(256):
                    ...
                    if padded:
                        expected_source[cursor * 32 : cursor * 32 + selected.numel()] = selected // 8
                        expected_positions[cursor * 32 : cursor * 32 + selected.numel()] = selected
    ...
    def test_paired_gemm1_swiglu_a4_matches_projection_and_codec(self):
        ...
        gate_up = Weight("nvfp4", gate_base.data, gate_base.logical_shape, gate_base.block_scale, weight_g, gate_g)
        down = Weight("nvfp4", down_base.data, down_base.logical_shape, down_base.block_scale, down_base.global_scale, down_g)
        ...
        source = (torch.arange(blocks.numel(), dtype=torch.int32).repeat_interleave(32) & 1).cuda()
        qz, captured_z = _paired_gemm1_swiglu_a4(qx, source, blocks, gate_up, down, capture_z=True)
        torch.cuda.synchronize()

        x_decoded = unpack_a4(qx.data.cpu(), qx.block_scale.cpu(), gate_g[:1].cpu()).float()
        expected = []
        for expert, token in ((3, 0), (17, 1)):
            ...
            z = (torch.nn.functional.silu(gu[:, :32].float()) * gu[:, 32:].float()).half()
            expected.append(z)
        ...
        nrmse = ((observed_z.float() - expected_z.float()).square().mean().sqrt() / expected_z.float().square().mean().sqrt()).item()
    ...
    def test_raw_grouped_gemm_unweighted_multiexpert_baseline(self):
        ...
        x = (torch.randn((2, 32), dtype=torch.float16) * 0.1).cuda()
        weight = _nvfp4_weight(torch.randn((256, 32, 32), dtype=torch.float16) * 0.03, device="cuda")
        qx = quantize_nvfp4(x, weight.input_global_scale[0])
        source = torch.cat((torch.zeros(32, dtype=torch.int32), torch.ones(32, dtype=torch.int32))).cuda()
        ...
        torch.cuda.synchronize()
        x_decoded = unpack_a4(qx.data.cpu(), qx.block_scale.cpu(), weight.input_global_scale[:1].cpu()).float()
        expected = []
        for expert, token in ((3, 0), (17, 1)):
            decoded_weight = unpack_a4(
                weight.data[expert].cpu(), weight.block_scale[expert].cpu(), weight.global_scale[expert : expert + 1].cpu()
            ).float()
            expected.append((x_decoded[token : token + 1] @ decoded_weight.T).half().expand(32, -1))
        expected = torch.cat(expected).cuda()
        nrmse = ((out.float() - expected.float()).square().mean().sqrt() / expected.float().square().mean().sqrt()).item()
        self.assertLess(nrmse, 2e-3)

    @unittest.skipUnless(REAL_NVFP4_CHECKPOINT.is_dir(), "real Qwen3.5 NVFP4 checkpoint is unavailable")
    def test_real_layer1_all_experts_balanced(self):
        ...
        gate_up, down = layer["mlp.experts.gate_up_proj"], layer["mlp.experts.down_proj"]
        ...
        x_decoded = unpack_a4(x_data, x_sf, gate_up.input_global_scale[:1].cpu()).float().cuda()
        expected = torch.zeros((32, 2048), dtype=torch.float32, device="cuda")
        for expert in range(256):
            token = expert // 8
            gate_up_decoded = unpack_a4(
                gate_up.data[expert].cpu(), gate_up.block_scale[expert].cpu(), gate_up.global_scale[expert : expert + 1].cpu()
            ).float().cuda()
            gu = (x_decoded[token : token + 1] @ gate_up_decoded.T).half()
            z = (torch.nn.functional.silu(gu[:, :512].float()) * gu[:, 512:].float()).half()
            z_data, z_sf, _ = quantize_a4(z.cpu(), down.input_global_scale[expert : expert + 1].cpu())
            z_decoded = unpack_a4(z_data, z_sf, down.input_global_scale[expert : expert + 1].cpu()).float().cuda()
            down_decoded = unpack_a4(
                down.data[expert].cpu(), down.block_scale[expert].cpu(), down.global_scale[expert : expert + 1].cpu()
            ).float().cuda()
            expected[token : token + 1] += (z_decoded @ down_decoded.T).half().float() * 0.125
        torch.cuda.synchronize()
        expected = expected.half()
        error = (out.float() - expected.float()).abs()
        nrmse = (error.square().mean().sqrt() / expected.float().square().mean().sqrt()).item()
    ...
    def test_real_layer0_fp16_all_experts_balanced_precision(self):
        ...
        for expert in range(256):
            ...
            z = (torch.nn.functional.silu(gu[:, :512].float()) * gu[:, 512:].float()).half()
            expected[token : token + 1] += (z.float() @ down.data[expert].float().T).half().float() * 0.125
        torch.cuda.synchronize()
        expected = expected.half()
        nrmse = ((out.float() - expected.float()).square().mean().sqrt() / expected.float().square().mean().sqrt()).item()
    ...
    def test_real_shared_expert_gate_fp16_sigmoid_boundary(self):
        ...
        shared = _fp16_swiglu(x, layer["mlp.shared_expert.gate_up_proj"], layer["mlp.shared_expert.down_proj"])
    ...
    def test_real_layer1_fused_moe_single_token_oracle(self):
        ...
        moe = MoeWeights(
            router=layer["mlp.gate"], gate_up=layer["mlp.experts.gate_up_proj"], down=layer["mlp.experts.down_proj"],
            shared_gate_up=layer["mlp.shared_expert.gate_up_proj"], shared_down=layer["mlp.shared_expert.down_proj"],
        ...
        for expert, route_weight in zip(expert_ids.tolist(), route_weights.tolist()):
            w1 = unpack_a4(gate_up.data[expert].cpu(), gate_up.block_scale[expert].cpu(), gate_up.global_scale[expert : expert + 1].cpu()).float().cuda()
            gu = (x_decoded @ w1.T).half()
            z = (torch.nn.functional.silu(gu[:, :512].float()) * gu[:, 512:].float()).half()
            z_data, z_sf, _ = quantize_a4(z.cpu(), down.input_global_scale[expert : expert + 1].cpu())
            z_decoded = unpack_a4(z_data, z_sf, down.input_global_scale[expert : expert + 1].cpu()).float().cuda()
            w2 = unpack_a4(down.data[expert].cpu(), down.block_scale[expert].cpu(), down.global_scale[expert : expert + 1].cpu()).float().cuda()
            routed += (z_decoded @ w2.T).half().float() * route_weight
        routed = routed.half()
        shared_gu = (x.float() @ moe.shared_gate_up.data.float().T).half()
        shared_z = (torch.nn.functional.silu(shared_gu[:, :512].float()) * shared_gu[:, 512:].float()).half()
        shared = (shared_z.float() @ moe.shared_down.data.float().T).half()
        shared_gate = (x.float() @ moe.shared_gate.data.float().T).half()
        shared = (shared.float() * torch.sigmoid(shared_gate.float()).half().float()).half()
