...
class TestExactHeaderManifest(unittest.TestCase):
    ...
    def test_generated_manifest_has_exact_required_counts(self):
        ...
        self.assertEqual(sum(spec.dtype == "F8_E4M3" and name.endswith(".weight") for name, spec in EXPECTED_HEADERS.items()), 130)
        self.assertEqual(sum(name.endswith("weight_packed") for name in EXPECTED_HEADERS), 29184)
        self.assertEqual(sum(spec.dtype == "F16" and ".mlp.experts." in name and name.endswith(".weight") for name, spec in EXPECTED_HEADERS.items()), 1536)
    ...
    def test_rejects_wrong_name_shape_and_dtype_before_payload_read(self):
        ...
        for malformed in (
            {"model.language_model.layers.3.self_attn.typo.weight": (spec.shape, "F8_E4M3")},
            {name: ((4096, 2048), "F8_E4M3")},
            {name: (spec.shape, "F16")},
            {"model.language_model.layers.3.self_attn.q_proj.weight_scale": ((64, 15), "F16")},
        ):
